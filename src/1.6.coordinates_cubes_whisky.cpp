// generate lots of cubes with constexpr stateless rng
...
static consteval std::array<glm::vec3, 1000> initCubePoss() {
  std::array<glm::vec3, 1000> res = {};
  for (size_t i = 0; i < std::size(res); ++i) {
    res[i].x = whisky1f(whisky1(3 * i)) * 20.0f - 10.0f;
    res[i].y = whisky1f(whisky1(3 * i + 1)) * 20.0f - 10.0f;
    res[i].z = whisky1f(whisky1(3 * i + 2)) * 20.0f - 10.0f;
  }
  return res;
}
static constexpr auto cubePoss = initCubePoss();
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    projection = glm::perspective(
        glm::pi<float>() * (0.5f + 0.4f * ::cos((float)glfwGetTime())),
